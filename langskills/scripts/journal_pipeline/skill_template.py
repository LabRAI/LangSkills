...
# File: skill_template.pyc (Python 3.12)
...
def build_journal_skill_markdown(*, title, source_url, run_id, source_artifact_id, doi, journal, journal_family, pub_date):
    ...
    lines.append('else')
    lines.append(f'''  RUN_DIR="${{RUN_DIR:-var/captures/{run_id}}}"''')
    lines.append(f'''  ARTIFACT="$RUN_DIR/sources/{source_artifact_id}.json"''')
    ...
    lines.append('  Unsupported opcode: MAP_ADD (188)
...
def write_journal_skill(*, run_dir, domain, seq, base_slug, paper_title, source_url, source_fetched_at, source_artifact_id, doi, journal, journal_family, pub_date, skill_kind, license_spdx, license_risk, language):
    ...
    if not str('').strip():
        str('').strip()
    sid = _make_skill_id(domain = str('').strip(), method = 'journal', seq = int(seq), base_slug = slugify(paper_title, 40))
    ...
    if not source_artifact_id:
        source_artifact_id
    skill_md = build_journal_skill_markdown(title = title, source_url = str('').strip(), run_id = str(Path(run_dir).name), source_artifact_id = str('').strip(), doi = doi, journal = journal, journal_family = journal_family, pub_date = pub_date)
