...
const LINUX_CAPTURE_SPECS = {
  ...
  "filesystem/copy-move-cp-mv": {
    sources: [
      {
        label: "man7 cp(1)",
        url: "https://man7.org/linux/man-pages/man1/cp.1.html",
        summary: "cp(1) 复制文件/目录与常用选项（-a/-r/-n/-i/--backup 等）。",
        supports: "Steps 1-4",
      },
      {
        label: "man7 mv(1)",
        url: "https://man7.org/linux/man-pages/man1/mv.1.html",
        summary: "mv(1) 移动/重命名与覆盖控制（-n/-i 等）。",
        supports: "Steps 5-6",
      },
      {
        label: "GNU coreutils manual (cp/mv)",
        url: "https://www.gnu.org/software/coreutils/manual/coreutils.html",
        summary: "GNU coreutils 对 cp/mv 行为与选项的详细说明（含备份策略）。",
        supports: "Steps 1-6",
      },
    ],
    goal: ["安全地复制/移动文件与目录，避免意外覆盖，并在需要时保留权限与时间戳。"],
    whenUse: ["需要复制文件/目录到新位置，或批量重命名/移动", "需要避免覆盖或需要保留权限/时间戳/符号链接语义"],
    whenNot: ["跨机器或大目录增量同步（优先用 `rsync`）", "目标路径不确定、可能覆盖关键文件（先做 dry-run 列表/备份）"],
    prerequisites: {
      environment: "Linux shell",
      permissions: "对源路径有读权限，对目标目录有写权限",
      tools: "`cp`, `mv`",
      inputs: "源路径、目标路径（以及是否允许覆盖/是否需要保留属性）",
    },
    steps: [
      { text: "复制单个文件并避免覆盖：`cp -n <src> <dst>`（必要时用 `-i` 交互确认）", cites: [1, 3] },
      {
        text: "复制目录并保留属性：`cp -a <src_dir> <dst_dir>`（比 `cp -r` 更安全，保留权限/时间戳/链接语义）",
        cites: [1, 3],
      },
      { text: "需要查看将复制什么时，用 `-v` 输出计划：`cp -av <src_dir> <dst_dir>`", cites: [1, 3] },
      { text: "要保留被覆盖前的旧版本，使用备份：`cp --backup=numbered <src> <dst>`（或 `--backup=t`）", cites: [1, 3] },
      { text: "移动/重命名时避免覆盖：`mv -n <old> <new>`（不确定时用 `mv -i`）", cites: [2, 3] },
      { text: "完成后验证：用 `ls -l`/`stat` 检查权限/时间戳；必要时用 `diff -q <src> <dst>` 抽样比对内容", cites: [1, 2, 3] },
    ],
    verification: ["检查是否意外覆盖：`ls -l <dst>`（时间戳/大小是否合理）", "抽样比对：`diff -q <src> <dst>` 或 `cmp <src> <dst>`"],
    safety: {
      irreversible: "覆盖（overwrite）可能导致数据丢失；移动会改变原路径位置",
      privacy: "避免把含密钥/凭据的文件复制到不安全位置或提交到版本库",
      confirmation: "默认用 `-n` 或 `-i`；关键路径先备份（`--backup=numbered`）再批量操作",
    },
    library: {
      bash: [
    ...
    references: {
      ...
      examples: [
        "```bash",
        "# Copy a directory tree while preserving permissions and timestamps",
        "cp -a ./project ./project.bak",
        "",
        "# Rename a file safely",
        "mv -n results.txt results_2026-01-15.txt",
        "```",
      ],
    },
  },

  "filesystem/checksums-cksum-sha256sum": {
    sources: [
      {
        label: "man7 sha256sum(1)",
      ...
      {
        label: "GNU coreutils manual (cksum/sha*sum)",
        url: "https://www.gnu.org/software/coreutils/manual/coreutils.html",
        summary: "coreutils 中 checksum 工具的用法与注意事项（含校验模式）。",
        supports: "Steps 1-5",
      },
    ],
    goal: ["用校验和验证文件是否被篡改/损坏，并能把校验结果落盘以便后续复核。"],
    whenUse: ["下载模型/数据集后需要验证完整性", "在备份/迁移后需要确认文件未变化"],
    whenNot: ["把 checksum 当作“安全证明”（它只能验证一致性，不能替代签名/可信发布渠道）", "对超大目录做全量校验但没有增量策略（先按关键文件/manifest 做）"],
    prerequisites: {
      environment: "Linux shell",
      permissions: "对目标文件有读权限",
      tools: "`sha256sum`（或同类 sha*sum）, `cksum`",
      inputs: "目标文件路径；可信渠道提供的 checksum（或自建 manifest）",
    },
    steps: [
      { text: "为单个文件生成 SHA-256：`sha256sum <file> > <file>.sha256`", cites: [1, 3] },
      { text: "从校验文件验证：`sha256sum -c <file>.sha256`（应输出 `OK`）", cites: [1, 3] },
      { text: "对多文件生成 manifest：`sha256sum <file1> <file2> > checksums.sha256`", cites: [1, 3] },
      { text: "需要可移植校验时用 POSIX `cksum`：`cksum <file>`（记录 CRC 与 bytes）", cites: [2] },
      { text: "对照可信渠道提供的 checksum（发布页/签名文件），确保算法一致（SHA-256 vs SHA-1 等）", cites: [1, 3] },
    ],
    verification: ["`sha256sum -c ...` 输出全部为 `OK`", "对同一文件重复计算得到相同 checksum（未被修改）"],
    safety: {
      irreversible: "无",
      privacy: "校验 manifest 可能暴露文件名与目录结构；共享前去除敏感路径",
      confirmation: "只从可信渠道获取 checksum/签名；不要在不可信页面复制粘贴可执行命令",
    },
    library: {
      bash: [
    ...
    references: {
      troubleshooting: [
        ...
        "- 确认文件未被解压/重命名导致路径不匹配。",
        "- 重新下载并再次校验；确认 checksum 来自可信渠道且算法一致。",
      ],
      edgeCases: [
      ...
      examples: [
        "```bash",
        "# Verify multiple files from a manifest",
  ...
  "filesystem/compress-gzip-xz": {
    sources: [
      {
        label: "Arch man: gzip(1)",
        url: "https://man.archlinux.org/man/gzip.1.en.txt",
        summary: "gzip/gunzip 常用选项（-k 保留原文件、-d 解压、-l 列表）。",
        supports: "Steps 1-3",
      },
      {
        label: "GNU gzip manual",
      ...
      {
        label: "Arch man: xz(1)",
        url: "https://man.archlinux.org/man/xz.1.en.txt",
        summary: "xz/xzcat 常用选项（-k 保留原文件、-d 解压、-T 并行）。",
        supports: "Steps 4-5",
      },
    ],
    goal: ["用 gzip/xz 压缩与解压文件，选择合适的参数，并在需要时保留原文件以便回滚。"],
    whenUse: ["需要压缩日志/数据文件以节省空间或便于传输", "需要解压 `.gz`/`.xz` 文件并保留原件"],
    whenNot: ["已经在 tar 里做了压缩且不需要二次压缩（避免浪费 CPU）", "对随机数据（加密/压缩过的文件）再次压缩（收益很低）"],
    prerequisites: {
      environment: "Linux shell",
      permissions: "对目标文件有读写权限（生成压缩文件）",
      tools: "`gzip`/`gunzip`, `xz`/`unxz`",
      inputs: "目标文件路径与期望的压缩格式（gz/xz）",
    },
    steps: [
      { text: "gzip 压缩且保留原文件：`gzip -k <file>`（生成 `<file>.gz`）", cites: [1, 2] },
      { text: "gzip 解压且保留压缩包：`gzip -dk <file>.gz`（等价 `gunzip -k`）", cites: [1, 2] },
      { text: "查看 gzip 压缩包信息：`gzip -l <file>.gz`（原始大小/压缩后大小）", cites: [1, 2] },
      { text: "xz 压缩且保留原文件：`xz -k <file>`（更高压缩率可加 `-9e`；并行可加 `-T0`）", cites: [3] },
      { text: "xz 解压且保留压缩包：`xz -dk <file>.xz`（等价 `unxz -k`）", cites: [3] },
    ],
    verification: ["解压后文件大小合理；必要时用 `sha256sum` 对比压缩前后的内容一致性", "`gzip -t <file>.gz` 可做完整性测试（通过则退出码为 0）"],
    safety: {
      irreversible: "默认 `gzip <file>` 会替换原文件；建议用 `-k` 保留原件",
      privacy: "压缩包仍包含原始数据；分享前确认不包含敏感信息",
      confirmation: "不确定时先用 `-k`；批量压缩前先对小样本验证输出与命名",
    },
    library: {
      bash: [
    ...
    references: {
      ...
      edgeCases: ["- `gzip -k` 在部分系统上可能是 GNU 扩展；如不可用，改用 `gzip -c <file> > <file>.gz`。"],
      examples: [
        "```bash",
        "# Stream gzip to stdout (keeps original by design)",
        "gzip -c ./big.txt > big.txt.gz",
        "```",
      ],
    },
  },
  ...
  "filesystem/mount-umount": {
    sources: [
      {
        ...
        summary: "lsblk(8) 列出块设备与文件系统信息（用于确认要挂载的分区）。",
        supports: "Step 1",
      },
      {
        label: "man7 mount(8)",
        url: "https://man7.org/linux/man-pages/man8/mount.8.html",
        summary: "mount(8) 挂载设备到目录（含 -o 选项、只读挂载等）。",
        supports: "Steps 2-4",
      },
      {
        label: "man7 umount(8)",
        url: "https://man7.org/linux/man-pages/man8/umount.8.html",
        summary: "umount(8) 卸载挂载点（含常见失败原因与选项）。",
        supports: "Steps 5-6",
      },
    ],
    goal: ["按安全流程挂载/卸载文件系统：先确认设备，再只读验证，再按需读写挂载，最后安全卸载。"],
    whenUse: ["需要临时挂载磁盘/分区/镜像查看内容", "需要挂载移动硬盘或云盘以读写文件"],
    whenNot: ["不确定设备节点（/dev/xxx）指向什么（先停下，避免挂错盘）", "生产环境在线挂载/卸载且不清楚影响范围（先评估依赖与回滚）"],
    prerequisites: {
      environment: "Linux shell",
      permissions: "通常需要 root 权限（`sudo`）进行 mount/umount",
      tools: "`lsblk`, `mount`, `umount`",
      inputs: "设备节点（例如 /dev/sdb1）与挂载点目录（例如 /mnt/data）",
    },
    steps: [
      { text: "确认设备与文件系统：`lsblk -f`（核对 SIZE/UUID/FSTYPE，避免挂错盘）", cites: [1] },
      { text: "创建挂载点目录：`sudo mkdir -p /mnt/data`", cites: [2] },
      { text: "先用只读挂载做验证：`sudo mount -o ro <device> /mnt/data`", cites: [2] },
      { text: "验证挂载成功：`mount | grep /mnt/data` 并查看可用空间：`df -h /mnt/data`", cites: [2] },
      { text: "需要读写时，先卸载只读再重新挂载：`sudo umount /mnt/data && sudo mount <device> /mnt/data`", cites: [2, 3] },
      { text: "完成后安全卸载：`sudo umount /mnt/data`（提示 busy 时先停止占用进程，再重试）", cites: [3] },
    ],
    verification: ["`mount | grep <mountpoint>` 能看到挂载记录；卸载后记录消失", "`df -h <mountpoint>` 能显示该挂载点的空间信息"],
    safety: {
      irreversible: "挂载到错误的设备/分区可能导致误操作；读写挂载下的删除/覆盖是不可逆的",
      privacy: "挂载介质可能包含敏感数据；确认访问权限与共享策略",
      confirmation: "任何读写操作前先只读挂载检查；在生产环境先确认依赖与回滚路径",
    },
    library: {
      bash: [
    ...
    references: {
      troubleshooting: [
        ...
        "- 关闭正在使用该挂载点的进程/终端；在确认影响后再重试卸载。",
        "- 不要轻易使用强制/懒卸载；除非你理解它的后果并有回滚计划。",
      ],
      edgeCases: ["- 如果挂载点包含空格/特殊字符，命令行引用要加引号。"],
      examples: [
        "```bash",
        "# Mount a disk image (loop) read-only (example)",
        "sudo mount -o loop,ro ./disk.img /mnt/data",
        "```",
      ],
    },
  },
  ...
  "text/sort-uniq": {
    sources: [
      ...
      {
        label: "GNU coreutils manual (sort/uniq)",
        url: "https://www.gnu.org/software/coreutils/manual/coreutils.html",
        summary: "coreutils 对 sort/uniq 的更完整说明（含 locale/稳定性等注意点）。",
        supports: "Steps 1-6",
      },
    ],
    goal: ["用 sort/uniq 做排序、去重、计数与按列排序，并避免常见陷阱（locale、uniq 需先排序）。"],
    whenUse: ["需要对日志/列表排序或去重", "需要统计重复项出现次数（Top-N）"],
    whenNot: ["数据量巨大且内存受限（考虑 `sort -T` 指定临时目录或用专门工具/数据库）", "需要稳定的区域性排序但不理解 locale 影响（先固定 `LC_ALL=C`）"],
    prerequisites: {
      environment: "Linux shell",
      permissions: "对输入文件有读权限；对输出路径有写权限",
      tools: "`sort`, `uniq`",
      inputs: "输入文件（每行一条记录）与排序/去重规则（数值/字典序/按列等）",
    },
    steps: [
      { text: "基础排序：`sort <in.txt> > sorted.txt`", cites: [1, 3] },
      { text: "数值排序：`sort -n numbers.txt`；人类可读数值（1K/10M）用 `sort -h`", cites: [1, 3] },
      { text: "按分隔符与列排序：`sort -t',' -k2,2 data.csv`（按第 2 列）", cites: [1, 3] },
      { text: "去重（推荐先排序）：`sort <in.txt> | uniq`；或直接用 `sort -u <in.txt>`", cites: [1, 2, 3] },
      { text: "统计重复次数：`sort <in.txt> | uniq -c | sort -nr`（Top-N 可再加 `head`）", cites: [2, 3] },
      { text: "locale 影响排序时，固定为字节序：`LC_ALL=C sort ...`（结果更可复现）", cites: [1, 3] },
    ],
    verification: ["用 `sort -c sorted.txt` 检查是否已排序（通过则退出码为 0）", "抽样查看排序/去重结果是否符合预期（尤其是按列排序）"],
    safety: {
      irreversible: "无（建议把输出重定向到新文件，避免覆盖原件）",
      privacy: "对包含敏感数据的日志做排序/统计时，避免把输出上传到公开渠道",
      confirmation: "如果要覆盖原文件，先写到临时文件并备份（例如 `mv -n`）",
    },
    library: {
      bash: [
    ...
    references: {
      ...
      edgeCases: ["- CSV 含引号/逗号转义时，简单 `-t','` 可能不可靠；需要真正的 CSV 解析器。"],
      examples: [
        "```bash",
  ...
  "text/cut-paste": {
    sources: [
      {
        ...
        summary: "cut(1) 按字符/字节/字段切分（-c/-b/-d/-f）。",
        supports: "Steps 1-3",
      },
      {
        label: "man7 paste(1)",
        url: "https://man7.org/linux/man-pages/man1/paste.1.html",
        summary: "paste(1) 把多文件按行拼接为列（-d 指定分隔符）。",
        supports: "Steps 4-5",
      },
      {
        label: "POSIX cut",
        url: "https://pubs.opengroup.org/onlinepubs/9699919799/utilities/cut.html",
        summary: "POSIX cut 规范（字段/字符选择语义与边界）。",
        supports: "Steps 1-3",
      },
    ],
    goal: ["用 cut/paste 对行文本做按列切分与拼接，快速处理简单 CSV/TSV/日志字段。"],
    whenUse: ["日志/TSV/简单 CSV 需要提取少数列", "需要把两个一列文件按行合并成多列表"],
    whenNot: ["复杂 CSV（引号、转义、嵌套逗号）需要真正的 CSV parser", "字段是结构化 JSON（优先用 jq 等）"],
    prerequisites: {
      environment: "Linux shell",
      permissions: "对输入文件有读权限；对输出路径有写权限",
      tools: "`cut`, `paste`",
      inputs: "输入文件与分隔符（如逗号/制表符），以及要提取/拼接的列号",
    },
    steps: [
      { text: "按分隔符提取字段：`cut -d',' -f1,3 data.csv`（取第 1 与第 3 列）", cites: [1, 3] },
      { text: "TSV（tab）常用：`cut -f2-4 data.tsv`（默认分隔符为 tab）", cites: [1, 3] },
      { text: "按字符位置切片：`cut -c1-12 file.txt`（例如取固定宽度前 12 个字符）", cites: [1, 3] },
      { text: "把两列文件按行拼接：`paste -d',' col1.txt col2.txt > merged.csv`", cites: [2] },
      { text: "把多行合并到一行：`paste -sd',' file.txt`（把所有行用逗号拼起来）", cites: [2] },
    ],
    verification: ["抽样检查输出列是否对齐（尤其是 CSV/TSV）", "对固定宽度切片，确认字段位置没有漂移（避免误切）"],
    safety: {
      irreversible: "无（建议输出到新文件）",
      privacy: "提取后的列可能仍包含敏感字段；共享前做脱敏/去标识化",
      confirmation: "对生产数据先用少量行（`head`）验证分隔符与列号正确，再跑全量",
    },
    library: {
      bash: [
    ...
    references: {
      troubleshooting: ["## Wrong delimiter", "- CSV 通常是 `,`，TSV 通常是 tab；先用 `head -n 2` 看实际分隔符。"],
      edgeCases: ["- cut 不理解 CSV 引号；包含逗号的字段会被误切。"],
      examples: [
        "```bash",
  ...
  "text/diff-patch": {
    sources: [
      {
        ...
        summary: "diff(1) 生成差异（-u 统一 diff），常用于代码审阅与补丁制作。",
        supports: "Steps 1-2,5",
      },
      {
        label: "POSIX diff",
        url: "https://pubs.opengroup.org/onlinepubs/9699919799/utilities/diff.html",
        summary: "POSIX diff 规范（基础行为与输出格式）。",
        supports: "Steps 1-2",
      },
      {
        label: "man7 patch(1)",
        url: "https://man7.org/linux/man-pages/man1/patch.1.html",
        summary: "patch(1) 应用/回滚补丁（--dry-run、-pN、-R 等）。",
        supports: "Steps 3-6",
      },
    ],
    goal: ["用 diff 生成统一补丁，用 patch 以 dry-run 方式安全应用，并能在出错时回滚。"],
    whenUse: ["需要对比两个文件/目录的差异并形成可应用的补丁", "在没有 git 的环境中应用变更（或对第三方补丁做审阅）"],
    whenNot: ["仓库已有 git 且可以直接开分支/PR（优先用 git 的 diff/apply/checkout）", "你不理解补丁影响范围（先做 dry-run 与备份）"],
    prerequisites: {
      environment: "Linux shell",
      permissions: "对目标文件有写权限（应用 patch）",
      tools: "`diff`, `patch`",
      inputs: "原文件/新文件路径；补丁要应用到的工作目录",
    },
    steps: [
      { text: "生成统一 diff：`diff -u old.txt new.txt > change.patch`", cites: [1, 2] },
      { text: "审阅补丁内容：`sed -n '1,120p' change.patch`（确认没有误改路径/大段无关变更）", cites: [1] },
      { text: "应用前先 dry-run：`patch --dry-run -p0 < change.patch`（应提示成功或给出冲突点）", cites: [3] },
      { text: "dry-run 通过后再应用：`patch -p0 < change.patch`", cites: [3] },
      { text: "验证应用结果：`diff -u old.txt new.txt` 应为空（或按预期变化）", cites: [1, 2] },
      { text: "需要回滚时：`patch -R -p0 < change.patch`（反向应用）", cites: [3] },
    ],
    verification: ["`patch --dry-run` 通过；应用后目标文件内容符合预期", "如是文本文件，可用 `diff -u` 再对比确认"],
    safety: {
      irreversible: "patch 会修改文件；在没有版本控制时风险更高",
      privacy: "补丁可能包含敏感路径/内容；共享前检查并脱敏",
      confirmation: "始终先 `--dry-run`；对关键文件先备份或在 git 分支上操作",
    },
    library: {
      bash: [
    ...
    references: {
      ...
      edgeCases: ["- 对目录做 diff 时可用 `diff -ruN old_dir new_dir > change.patch`（更复杂，先 dry-run）。"],
      examples: [
        "```bash",
  ...
  "text/xargs-basics": {
    sources: [
      {
        ...
        summary: "POSIX xargs 规范（从 stdin 构造命令行参数）。",
        supports: "Steps 1-4",
      },
      {
        label: "man7 xargs(1)",
        url: "https://man7.org/linux/man-pages/man1/xargs.1.html",
        summary: "xargs(1) 常用选项（-0/-n/-I/-P 等）与安全注意点。",
        supports: "Steps 1-6",
      },
      {
        label: "GNU findutils manual (find)",
        url: "https://www.gnu.org/software/findutils/manual/html_mono/find.html",
        summary: "findutils 手册（含 find 与 xargs 的常见组合与注意事项，如 -print0/-0）。",
        supports: "Steps 4-6",
      },
    ],
    goal: ["用 xargs 把 stdin 的多行输入转成命令参数，并用 -0/-n/-P 等选项提高安全性与可控性。"],
    whenUse: ["需要把一列路径/ID 批量喂给某个命令", "需要限制每次参数数量或并行执行"],
    whenNot: ["输入包含复杂转义且无法保证分隔符（优先用 `-print0`/`-0` 或改用 `-exec ... {} +`）", "对不可逆命令做批量执行但没有预览（先 echo/dry-run）"],
    prerequisites: {
      environment: "Linux shell",
      permissions: "取决于将要执行的命令（xargs 本身只读 stdin）",
      tools: "`xargs`（以及将要运行的目标命令）",
      inputs: "输入列表（stdin）、目标命令、每次处理多少项、是否并行",
    },
    steps: [
      { text: "基础用法：`printf '%s\\\\n' a b c | xargs echo`", cites: [1, 2] },
      { text: "限制每次参数数量：`printf '%s\\\\n' a b c | xargs -n 1 echo`", cites: [1, 2] },
      { text: "使用占位符：`printf '%s\\\\n' a b | xargs -I{} echo \"item={}\"`", cites: [2] },
      { text: "处理包含空格的路径：`find . -type f -print0 | xargs -0 -n 1 echo`（搭配 -print0/-0）", cites: [2, 3] },
      { text: "并行（谨慎）：`printf '%s\\\\n' a b c | xargs -P 4 -n 1 echo`（确保目标命令可并发）", cites: [2] },
      { text: "对不可逆命令先预览：把 `echo` 换成真实命令前，先确认输出与数量（例如先 `wc -l`）", cites: [2] },
    ],
    verification: ["先用 `echo` 验证将执行的命令参数是否正确", "对实际命令执行前后统计输入/输出数量（例如 `wc -l`）"],
    safety: {
      irreversible: "xargs 常用于执行命令，危险性取决于目标命令（rm/chmod 等）",
      privacy: "stdin 列表可能包含敏感路径/ID；记录日志时注意脱敏",
      confirmation: "先 echo；优先使用 `-0` 处理路径；对高风险命令要求交互确认或小批量执行",
    },
    library: {
      bash: [
        "# Preview first",
    ...
    references: {
      ...
      edgeCases: ["- `xargs -I{}` 会禁用部分批处理优化；不需要占位符时优先用 `-n`。"],
      examples: [
        "```bash",
  ...
  "process/pkill-killall": {
    sources: [
      ...
      {
        label: "man7 killall(1)",
        url: "https://man7.org/linux/man-pages/man1/killall.1.html",
        summary: "killall 按进程名终止（同名进程较多时风险更高）。",
        supports: "Steps 2-5",
      },
    ],
    goal: ["用信号（TERM→KILL）安全结束目标进程，避免误杀，并在必要时收集信息以便排查。"],
    whenUse: ["某个进程卡死/占用资源，需要优雅终止再强制终止", "需要按进程名批量结束（谨慎）"],
    whenNot: ["不确定匹配规则会命中哪些进程（先用 pgrep/ps 预览）", "生产环境关键服务（先走维护窗口与回滚）"],
    prerequisites: {
      environment: "Linux shell",
      permissions: "通常只能终止自己拥有的进程；系统服务可能需要 sudo",
      tools: "`pgrep`/`pkill`, `kill`, `killall`",
      inputs: "目标进程名或 PID，以及期望的终止方式（优雅/强制）",
    },
    steps: [
      { text: "先确认会命中哪些进程：`pgrep -a '<pattern>'`（只列出，不终止）", cites: [2] },
      { text: "如需按名称终止，优先用 pkill 且先 TERM：`pkill -TERM -f '<pattern>'`", cites: [2] },
      { text: "按 PID 终止：`kill -TERM <pid>`（优雅退出）", cites: [1] },
      { text: "等待片刻后检查是否仍存在：`pgrep -a '<pattern>'` 或 `ps -p <pid>`", cites: [2] },
      { text: "仍不退出再升级为 KILL：`kill -KILL <pid>` 或 `pkill -KILL -f '<pattern>'`（最后手段）", cites: [1, 2] },
      { text: "谨慎使用 killall：确认进程名唯一，再执行 `killall -TERM <name>`", cites: [3] },
    ],
    verification: ["目标 PID/进程名不再出现在 `ps`/`pgrep` 输出中", "服务需要重启时，确认恢复策略（systemd/容器/脚本）"],
    safety: {
      irreversible: "KILL 会强制终止，可能导致数据丢失/状态损坏；误杀可能影响系统/服务",
      privacy: "排查日志可能包含用户数据；共享前脱敏",
      confirmation: "始终先 pgrep/ps 预览；优先 TERM；KILL 只作为最后手段",
    },
    library: {
      bash: [
        "# Preview targets",
    ...
    references: {
      troubleshooting: ["## KILL still doesn't work", "- 进程可能处于不可中断状态（D state）；需要排查 IO/内核问题或重启。"],
      edgeCases: ["- `-f` 会匹配完整命令行，容易误伤；能用更精确模式就更精确。"],
      examples: [
        "```bash",
        "# Stop a process by PID safely",
        "kill -TERM 12345",
        "sleep 2",
        "kill -KILL 12345",
        "```",
  ...
  "system/env-path": {
    sources: [
      {
        ...
        summary: "env(1) 查看/设置环境变量并运行命令（临时环境）。",
        supports: "Steps 1-3",
      },
      {
        label: "POSIX env",
        url: "https://pubs.opengroup.org/onlinepubs/9699919799/utilities/env.html",
        summary: "POSIX env 规范（可移植语义）。",
        supports: "Steps 1-3",
      },
      {
        label: "GNU Bash manual",
        url: "https://www.gnu.org/software/bash/manual/bash.html",
        summary: "Bash 手册（export、变量展开、PATH 解析等）。",
        supports: "Steps 4-6",
      },
    ],
    goal: ["查看与管理环境变量（尤其是 PATH），支持临时设置与持久化，并避免泄露敏感变量。"],
    whenUse: ["命令找不到（PATH 问题）或需要临时注入配置（ENV vars）", "需要为单条命令设置不同的变量值（不污染全局）"],
    whenNot: ["需要长期系统级配置但不了解 shell 初始化文件（先确认 bash/zsh 与加载顺序）", "变量包含凭据且准备截图/复制到公开渠道（先脱敏）"],
    prerequisites: {
      environment: "Linux shell (bash/zsh)",
      permissions: "一般不需要特殊权限；修改系统级配置可能需要 sudo",
      tools: "`env`, shell builtin `export`",
      inputs: "要查看/设置的变量名（例如 PATH/HTTP_PROXY）与作用范围（一次命令/当前会话/持久化）",
    },
    steps: [
      { text: "查看当前环境变量：`env | sort`（不要把输出直接发到公开渠道）", cites: [1, 2] },
      { text: "查看某个变量：`echo \"$PATH\"` 或 `env | rg '^PATH='`", cites: [3] },
      { text: "为单条命令临时设置变量：`VAR=value command ...` 或 `env VAR=value command ...`", cites: [1, 2] },
      { text: "在当前 shell 会话生效：`export VAR=value`（新开终端不会保留）", cites: [3] },
      { text: "追加 PATH（避免覆盖）：`export PATH=\"$PATH:/opt/bin\"`", cites: [3] },
      { text: "验证生效：`command -v <tool>` 或 `which <tool>`（确认解析到期望路径）", cites: [3] },
    ],
    verification: ["`command -v <tool>` 指向期望路径", "重新开一个 shell 后仍生效（如果做了持久化配置）"],
    safety: {
      irreversible: "错误修改 PATH 可能导致命令解析异常；通常可通过恢复配置回滚",
      privacy: "env 输出可能包含 token/密钥；共享前必须清理敏感变量",
      confirmation: "改动前先记录原值；优先以追加方式修改 PATH，而不是覆盖",
    },
    library: {
      bash: [
    ...
    references: {
      troubleshooting: [
        "## Command not found",
        "- 用 `command -v <tool>` 检查是否在 PATH 中。",
        "- 确认你改的是正确的 shell 初始化文件（bashrc/zshrc 等）。",
      ],
      edgeCases: ["- 在 sudo 环境下 PATH/ENV 可能被重置；需要区分用户 shell 与 root 环境。"],
      examples: [
        "```bash",
  ...
  "system/date-time-tz": {
    sources: [
      {
        ...
        summary: "date(1) 时间显示与格式化（+FORMAT，-u 等）。",
        supports: "Steps 1-4",
      },
      {
        label: "POSIX date",
        url: "https://pubs.opengroup.org/onlinepubs/9699919799/utilities/date.html",
        summary: "POSIX date 规范（可移植格式化语义）。",
        supports: "Steps 1-4",
      },
      {
        label: "GNU coreutils manual (date)",
        url: "https://www.gnu.org/software/coreutils/manual/coreutils.html",
        summary: "GNU date 扩展与常用格式示例（ISO-8601、epoch 等）。",
        supports: "Steps 1-5",
      },
    ],
    goal: ["用 date 查看/格式化时间，临时切换时区（TZ），并做基础一致性检查（UTC vs local）。"],
    whenUse: ["日志排查需要对齐时间窗口（UTC/local）", "需要生成可排序的时间戳（ISO-8601/epoch）"],
    whenNot: ["需要修改系统时间（高风险，涉及 NTP/服务影响；需额外流程）", "需要高精度时间同步（应使用专门的 NTP/chrony 工具与监控）"],
    prerequisites: {
      environment: "Linux shell",
      permissions: "查看时间无需权限；修改系统时间通常需要 sudo",
      tools: "`date`",
      inputs: "需要的输出格式（ISO/epoch/自定义）与时区（local/UTC/指定 TZ）",
    },
    steps: [
      { text: "查看当前本地时间：`date`", cites: [1, 2, 3] },
      { text: "输出 ISO-8601：`date -Iseconds`（更适合日志与机器处理）", cites: [1, 3] },
      { text: "查看 UTC：`date -u` 或 `TZ=UTC date`", cites: [1, 2, 3] },
      { text: "自定义格式：`date '+%Y-%m-%d %H:%M:%S %z'`", cites: [1, 2, 3] },
      { text: "临时切换时区：`TZ=America/New_York date`（只影响该命令）", cites: [3] },
    ],
    verification: ["对同一时刻，UTC 与 local 的差值符合预期时区偏移（%z）", "用于文件命名的时间戳排序正确（字符串排序即时间顺序）"],
    safety: {
      irreversible: "查看时间无；但修改系统时间会影响任务调度与证书校验等（不要在这里做）",
      privacy: "时间戳通常无隐私，但与其他信息组合可能暴露活动时间",
      confirmation: "只做查看与格式化；如要改系统时间，必须走额外审批/回滚流程",
    },
    library: {
      bash: [
        "# ISO timestamp",
    ...
    references: {
      troubleshooting: ["## Time looks wrong in logs", "- 确认日志是否以 UTC 记录；用 `date -u` 对齐时间窗口。"],
      edgeCases: ["- DST 切换会导致本地时间不连续；跨时区排查优先用 UTC。"],
      examples: [
        "```bash",
  ...
  "system/permissions-umask": {
    sources: [
      {
        label: "POSIX umask",
        url: "https://pubs.opengroup.org/onlinepubs/9699919799/utilities/umask.html",
        summary: "POSIX umask 语义与常见写法（八进制/符号）。",
        supports: "Steps 1-4",
      },
      {
        label: "man7 umask(1p)",
        url: "https://man7.org/linux/man-pages/man1/umask.1p.html",
        summary: "umask(1p) 命令（POSIX）与常见用法示例。",
        supports: "Steps 1-4",
      },
      {
        label: "GNU Bash manual",
        url: "https://www.gnu.org/software/bash/manual/bash.html",
        summary: "Bash 内建 umask 与 shell 初始化行为（如何持久化）。",
        supports: "Steps 5-6",
      },
    ],
    goal: ["理解并设置默认文件/目录权限（umask），用可验证的方法确认新建文件权限符合预期。"],
    whenUse: ["需要提升默认安全性（例如把新建文件默认变成 600/700）", "排查新建文件权限不对的问题"],
    whenNot: ["你不确定系统/应用依赖默认权限（先在隔离环境验证）", "把 umask 当作替代 ACL/权限管理（它只影响新建，不影响已有文件）"],
    prerequisites: {
      environment: "Linux shell",
      permissions: "一般不需要 sudo；系统级配置可能需要管理员权限",
      tools: "shell builtin `umask`",
      inputs: "期望的默认权限策略（例如 022/027/077）",
    },
    steps: [
      { text: "查看当前 umask：`umask`（通常输出 022/027/077 等八进制）", cites: [1, 2] },
      { text: "解释规则：umask 是要屏蔽的权限位；新文件默认 666、新目录默认 777，再减去 umask", cites: [1, 2] },
      { text: "临时设置为常见值：`umask 022`（更宽松）或 `umask 077`（更严格）", cites: [1, 2] },
      { text: "验证：`rm -f t && touch t && stat -c '%a %n' t`（看新文件权限是否符合预期）", cites: [2] },
      { text: "持久化到 shell：把 `umask 027`（或你的值）写入 `~/.bashrc`/`~/.profile`（按登录类型选择）", cites: [3] },
      { text: "重新登录或 `source` 配置后再次验证：`umask && touch t2 && stat -c '%a %n' t2`", cites: [3] },
    ],
    verification: ["新建文件/目录权限符合预期（例如 022→644/755，077→600/700）", "重启 shell 后仍保持（如果做了持久化）"],
    safety: {
      irreversible: "更改 umask 会影响之后新建文件权限，可能导致程序无法写入或协作失败",
      privacy: "更严格的 umask（如 077）可减少误共享；但也可能阻碍组协作",
      confirmation: "先在当前 shell 验证，再持久化；对团队协作环境先确认组权限策略",
    },
    library: {
      bash: [
        "# Check current umask",
    ...
    references: {
      troubleshooting: ["## Programs can't write files after changing umask", "- 恢复到更宽松值（例如 022/027），或为特定目录设置组权限/ACL。"],
      edgeCases: ["- 不同 shell 的初始化文件加载顺序不同；确认你写入的是正确文件（login shell vs interactive）。"],
      examples: [
        "```bash",
        "# Common choices",
        "umask 022  # typical default on many systems",
